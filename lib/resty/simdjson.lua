...
ngx.update_time()
local start = ngx.now()
local decoder = _M.new(true)
simdres = decoder:decode(json)


ngx.update_time()
print("decoder.decode: ", ngx.now() - start)
end
...
for i = 1, 10 do
    collectgarbage()
ngx.update_time()
cjres = cj.decode(json)
local now = ngx.now()
ngx.update_time()
print("cjson took: ", ngx.now() - now)
end

print("results are equal? ", equals(cjres, simdres))
